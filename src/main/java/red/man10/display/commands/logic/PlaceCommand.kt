...
class PlaceCommand(private var plugin: JavaPlugin) : CommandExecutor {
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        try {
            ...
            if(Main.displayManager.installingPlayer != null){
                sender.sendMessage(Main.prefix + "§c§l The display is being installed by ${Main.displayManager.installingPlayer?.name}")
                return false
            }
